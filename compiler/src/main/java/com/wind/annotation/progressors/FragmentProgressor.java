...
public class FragmentProgressor extends AbstractProcessor {
    /**
     * 元素操作的辅助类
     * */
    ...
    public Set<String> getSupportedAnnotationTypes() {
        ...
        set.add(InjectFragment.class.getCanonicalName());
        return set;
    }

    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        Set<? extends Element> elements = roundEnv.getElementsAnnotatedWith(InjectFragment.class);
        for (Element element : elements) {
            //没有Holder注解则继续循环
            InjectFragment injectFragmentAnnotation = element.getAnnotation(InjectFragment.class);
            if (null == injectFragmentAnnotation) {
                continue;
            }
            //生成类名
